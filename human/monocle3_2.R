...
saveRDS(sc_integr, file = "sce_monocle.rds")
...
##创建CDS对象并预处理数据
sce.harm<- readRDS("D:/aJMML/scRNA1/sce_monocle.rds")
pbmc<-sc_integr
...
plot_cells(cds, label_groups_by_cluster=FALSE,  color_cells_by = "seurat_clusters")#seurat_clusters
ggsave("Reduction.pdf",width = 10, height = 10)
#可视化指定基因
ciliated_genes <- c( "Slamf1","Kit","Cd34","Sca1","Cd48","Procr","Ifitm1","Mecom", "Hoxa9", "Mycn", "Hlf")
plot_cells(cds,
...
p = plot_cells(cds, color_cells_by = "pseudotime", label_cell_groups = FALSE, 
               label_leaves = FALSE,  label_branch_points = FALSE)
ggsave("Trajectory_Pseudotime2.pdf", plot = p, width = 8, height = 6)
