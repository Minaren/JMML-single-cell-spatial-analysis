...
library(GEOquery)
#下载数据，如果文件夹中有会直接读入
gset = getGEO('GSE71449', destdir=".", AnnotGPL = F, getGPL = F)
class(gset)
###提取子集
gset[[1]]
...
library(stringr)
#设置参考水平
group_list <- ifelse(str_detect(pdata$title, "Patient"), "JMML",
                     "normal")
#因子型
group_list = factor(group_list,
                    levels = c("normal","JMML"))
...
boxplot(exp,outline=FALSE, notch=T,col=group_list, las=2)
range(exp)#一般绝对值在20以内的就是经过注释的，所以不需要后面的步骤
#exp <- log2(exp+1)
#range(exp)
dev.off()

#手动注释#
GPL=getGEO(filename = 'GSE71449_family.soft.gz')
...
save(KEGG,KEGG_result_df,file = "GSEA_deg_JMML.rda")
