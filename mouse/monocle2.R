...
BiocManager::install("monocle", version = "3.21")
library(monocle)
library(dplyr)
library(Seurat)
library(patchwork)
...
expr_matrix <- as(as.matrix(sce.harm@assays$RNA@counts), 'sparseMatrix')
##提取表型信息到p_data(phenotype_data)里面 
p_data <- sce.harm@meta.data 
p_data$celltype <- sce.harm@active.ident  ##整合每个细胞的细胞鉴定信息到p_data里面。如果已经添加则不必重复添加
##提取基因信息 如生物类型、gc含量等
f_data <- data.frame(gene_short_name = row.names(sce.harm),row.names = row.names(sce.harm))
...
#expressed_genes <- VariableFeatures(sce.harm) 
...
pdf("train_monocle_celltype.pdf",width =10 ,height = 10)
...
ggplot(df, aes(Pseudotime, colour = orig.ident, fill=orig.ident)) +
  geom_density(bw=0.5,size=1,alpha = 0.5)+theme_classic2()
ggsave("时间_密度_分组.pdf",width = 30,height = 20,units = "cm")
