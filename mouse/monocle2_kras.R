...
Kras <- subset(sce.harm, orig.ident == "Kras")
##提取表型信息--细胞信息(建议载入细胞的聚类或者细胞类型鉴定信息、实验条件等信息)
expr_matrix <- as(as.matrix(Kras@assays$RNA@counts), 'sparseMatrix')
##提取表型信息到p_data(phenotype_data)里面 
p_data <- Kras@meta.data 
p_data$celltype <- Kras@active.ident  ##整合每个细胞的细胞鉴定信息到p_data里面。如果已经添加则不必重复添加
##提取基因信息 如生物类型、gc含量等
f_data <- data.frame(gene_short_name = row.names(Kras),row.names = row.names(Kras))
...
#expressed_genes <- VariableFeatures(Kras) 
...
head(deg)
##差异基因的结果文件保存
write.table(deg,file="train.monocle_DEG_Kras.xls",col.names=T,row.names=F,sep="\t",quote=F)

## 轨迹构建基因可视化
deg<-read.table("train.monocle.DEG.xls",sep="\t")
deg<-deg[-1,]
ordergene <- deg$V5
...
#以pseudotime值上色 
pdf("train.monocle.pseudotime_kras.pdf",width = 7,height = 7)
plot_cell_trajectory(cds,color_by="Pseudotime", size=1,show_backbone=TRUE) 
dev.off()
#以细胞类型上色
pdf("train.monocle.celltype_kras.pdf",width = 7,height = 7)
plot_cell_trajectory(cds,color_by="celltype", size=1,show_backbone=TRUE)
dev.off()
#以细胞状态上色
pdf("train.monocle.state_kras.pdf",width = 7,height = 7)
plot_cell_trajectory(cds, color_by = "orig.ident",size=0.05,show_backbone=TRUE)
dev.off()

#按照seurat分群排序细胞
pdf("seurat.clusters_kras.pdf",width = 7,height = 7)
plot_cell_trajectory(cds, color_by = "seurat_clusters")
dev.off()

#以细胞状态上色（拆分）“分面”轨迹图，以便更容易地查看每个状态的位置。
pdf("train_monocle_celltype_kras.pdf",width =30 ,height = 10)
