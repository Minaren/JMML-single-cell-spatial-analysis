...
sc_WT <- scRNAlist[[1]]
table(sc_WT$orig.ident) 
head(sc_WT@meta.data,5)
save(sc_WT,file = 'sc_WT.Rdata')
...
maxGene=6000#改成6000
pctMT=10
scRNA<- subset(scRNA1, subset = nFeature_RNA > minGene & nFeature_RNA < maxGene & 
                   percent.mt < pctMT) 
#过滤后的feature图
...
DimHeatmap(object = scRNA, dims = 1:30, cells = 500, balanced = TRUE,nfeatures = 30,ncol=3)
dev.off()

#PCA可视化4: JackStrawPlot 确定选多少个pc
scRNA <- JackStraw(object = scRNA, num.replicate = 100)
scRNA <- ScoreJackStraw(object = scRNA, dims = 1:15)
pdf(file="06.pcaJackStraw.pdf",width=8,height=6)
JackStrawPlot(object = scRNA, dims = 1:15)
dev.off()
...
dev.off()
save(scRNA,file = "sc_WT.RData")

####9.寻找差异表达的特征####
rm(list = ls())
load("D:/aJMML/scRNA1/sc_WT.RData")
genes_to_check =c("Prss34","Cd200r3","Lmo4",
                  ...
                  "Lmo4","Runx1",
                  "Slamf1","Kit","Cd34","Ly6a","Cd48","Procr","Ifitm1","Mecom", "Hoxa9", "Mycn", "Hlf","Cd201","Cd47",
                  "Ms4a1","Cd79a",
                  "Il7r","Ccr7","Cd8","Cd3d","Cd4","Gzmk","Fgfr1","Mac1","Ly6g","Fcgr3")
...
ggsave("vlnplot_before_qc_WT.pdf", plot = violin, width = 9, height = 8)
...
for(i in seq_along(plot.featrures)){
  plots[[i]] = VlnPlot(sc_filt, group.by=group, pt.size = 0, 
                       features = plot.featrures[i]) + theme.set2 + NoLegend()} 
violin <- wrap_plots(plots = plots, nrow=2) 
ggsave("vlnplot_after_qc_WT.pdf", plot = violin, width = 10, height = 8)
...
####2.5 降维及可视化####
sce.harm = RunUMAP(sc_n, dims = pc.num,reduction = "harmony")
...
celltype[celltype$ClusterID %in% c(1,6),2] = "MPP"
...
clusterCornerAxes(object = sce.harm,reduction = 'umap',clusterCol = 'seurat_clusters',pSize = 0.1,cellLabel = T,cellLabelSize = 5,
                  noSplit = T) 
ggsave("umap_mouse_umap.pdf",width = 10,height = 9,dpi = 500)
ggsave("umap.png",width = 10,height = 9,dpi = 500)
...
VlnPlot(sce.harm, features = c("Cd69"),pt.size=0,split.by = "orig.ident")
FeaturePlot(HSC,features = c("Cd69"),min.cutoff =0, max.cutoff = 10)
